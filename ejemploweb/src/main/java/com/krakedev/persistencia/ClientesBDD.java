...
public class ClientesBDD {
	...
	public Cliente buscarPorPK(String cedulaBusqueda) throws KrakeDevException {
		...
		try {
			...
			if (rs.next()) {
				System.out.println("Exise el cliente");
				String cedula = rs.getString("cedula");
				String nombre = rs.getString("nombre");
				int numeroHijos = rs.getInt("numeroHijos");
				cliente = new Cliente(cedula, nombre, numeroHijos);
			}else {
				System.out.println("No existe en cliente");
			}
		} catch (KrakeDevException e) {
			e.printStackTrace();
			throw e;
	...
	public List<Cliente> buscarPorNH(int numeroDeHijos) throws KrakeDevException {
		...
		try {
		    ...
		    while (rs.next()) {
		        ...
		        String nombre = rs.getString("nombre");
		        int numeroHijos = rs.getInt("numeroHijos");
		        clientes.add(new Cliente(cedula, nombre, numeroHijos));
		    }
		} catch (KrakeDevException e) {
		    e.printStackTrace();
		    throw e;
